...
let toCartBtn = document.getElementsByClassName('toCart');
...
let onPizzaClick = function (id) {
  ...
  modal.style.display = "block";
    modalContent.innerHTML = `  <div class ="modal_pizza_inner">    
                                <img class="modal_pizza-img" src="${pizzaObj.isNew ? '': 'img/'}${pizzaObj.img}" alt="pizza">
...
let showPizzaList = function () {
  ...
  filteredPizzaList.forEach((pizza, pizzaIndex) => {
    pizzaCards +=       `   <div class="pizza col-md-6 col-lg-4" id="pizza${pizzaIndex}">
                            ...
                            <img class="pizza-img" src="${pizza.isNew ? '': 'img/'}${pizza.img}" alt="pizza">
                            ...
                            <button class="toCart icon-basket" id="${pizza.id}" type="button" onclick="pizzaToCart(${pizza.id})">В корзину</button>
                            </div>`;              
  });
